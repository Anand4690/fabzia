(function () {
  ...
  const state = {
    ...
    hasChanged: false, // Track if any user input has occurred
  };
  ...
  function populateSportsDropdown() {
    SPORTS_LIST.forEach((sport) => {
      ...
      btn.className =
        "dropdown-option w-full px-4 py-3 text-left text-[var(--text-body)] font-inter text-[16px] font-medium hover:bg-gray-50 transition-colors";
  ...
  function setupEventListeners() {
    ...
    document.addEventListener("click", (e) => {
      if (
        ...
        !sportDropdownBtn.contains(e.target) &&
        !sportDropdownList.contains(e.target)
      ) {
  ...
  function handleUpdate() {
    ...
    if (!state.email || state.email.trim().length === 0) {
      const emailErrorText = emailError
        ? emailError.querySelector(".error-text")
        : null;
    ...
    // We check either the hasChanged flag or compare specific fields if needed
    if (!state.hasChanged) {
